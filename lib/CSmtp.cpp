...
void CSmtp::FormatHeader(char* header)
{
	...
	{ // no attachments
		...
		else if(m_headflag == 2)
		{
			strcat(header,"Content-type: text/html; charset=\"gb2312\"\r\n");
			strcat(header,"Content-Transfer-Encoding: base64\r\n");
		}
		else
		{
...
void CSmtp::SetHeadFlag(int flag)
{
	m_headflag = flag;
}

void CSmtp::CloseHandle()
{
	if (hSocket > 0)
		close(hSocket);
	hSocket = -1;
...
void CSmtp::SetMsgBody(const char *src)
{
	m_msgbody = src;
}
